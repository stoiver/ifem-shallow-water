function planardam_adaptive
    ...
    %---------------------- Initial Grid
    %--------------------------------------
    %node = [0 0; 1 0; 1 1; 0 1; 0.25 0.25; 0.75 0.75];
    %elem = [5 1 2; 6 5 2; 6 2 3; 5 4 1; 6 4 5; 6 3 4];
...
%      bla
    ...
    maxRefineCoarsen = 1;%2 
    dt = 0.002;%tol_El/2^(maxRefineCoarsen+3);%(tol_El/4)/(2^(maxRefineCoarsen+1));
    dx = dt;
    maxIt = 50;%100
    ...
    for i = 1:maxIt  
        ...
        for iterate=1:maxRefineCoarsen
            ...
            for j = 1:nt
                ...
                for k = 1:3
                    ...
                    U_prev_j_rot = rotate(U_prev_j, normal(1), normal(2));
                    h_prev_j = U_prev_j_rot(1,1);
                    hu_prev_j = U_prev_j_rot(2,1);

                    U_now_j_rot = rotate(U_now_j, normal(1), normal(2));
                    h_now_j = U_now_j_rot(1,1);
                    hu_now_j = U_now_j_rot(2,1);            


                    U_prev_jp1 = U_prev(:,neighbor);             
                    U_prev_jp1_rot = rotate(U_prev_jp1, normal(1), normal(2));
                    h_prev_jp1 = U_prev_jp1_rot(1,1);
                    hu_prev_jp1 = U_prev_jp1_rot(2,1);     
                    % Use reflective boundary at the moving water
                    if neighbor==j
                        if hu_prev_jp1 ~= 0.0
                            hu_prev_jp1 = -hu_prev_jp1;
                        end
                    end            

                    U_now_jp1 = U_now(:,neighbor);
                    U_now_jp1_rot = rotate(U_now_jp1, normal(1), normal(2));
                    h_now_jp1 = U_now_jp1_rot(1,1);
                    hu_now_jp1 = U_now_jp1_rot(2,1);   
                    % Use reflective boundary at the moving water
                    if neighbor==j
                        if hu_now_jp1 ~= 0.0
                            hu_now_jp1 = -hu_now_jp1;
                        end
                    end            

                    val_CK = 0.5*dx*(h_now_j - h_prev_j + h_now_jp1 - h_prev_jp1)+ 0.5* dt*(hu_prev_jp1 - hu_prev_j + hu_now_jp1 - hu_now_j);  
                    if abs(CK_temp) >= abs(val_CK)
                        CK_temp = CK_temp;
                    else
                        CK_temp = val_CK;
                    end
            ...
            tol_NEP = 0.01*max(NEP);
            %refineElem  = find(NEP >= tol_NEP);
            refineElem  = find(abs(CK) >= tol_CK);
            ...
            if isempty(brother) == 0  %0 means that brother is not empty 
                ...
                CKR = zeros(1,ntr); %storage initiation for CK on refined grids                
                for ii=1:ntr
                    UR(:,ii)   = Q(:,parents(1,ii));
                    %NEPR(1,ii) = NEP(1,parents(1,ii));
                    CKR(1,ii) = CK(1,parents(1,ii));
                end
            else
                UR = Q;
                %NEPR = NEP;
            ...
            coarsenElem = find(abs(CKR) < tol_CK);
            ...
            [node,elem,bdEdge,brother,UC,CKC] = coarsen_sudi(node,elem, markedElem, bdEdge, UR, CKR);
        ...
        subplot(1,2,2);
        showsolution(node,elem,CK/max(abs(CK))); view(3); 
...
% 
%             val_CK = 0.5*dx*(h_now_j - h_prev_j + h_now_jp1 - h_prev_jp1)+ 0.5* dt*(hu_prev_jp1 - hu_prev_j + hu_now_jp1 - hu_now_j);               
%             CK_temp = max(CK_temp, abs(val_CK));
%        end
%        CK(1,j) = CK_temp;       
%     end    
%     
...
%     patch(x',y',[CK;CK;CK],tcolor); pause(0.001); view(3)%colorbar; 
