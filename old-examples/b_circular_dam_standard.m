function circulardamstandard_triangular
    % Circular Dam Break
    %--------------------------------------------------------------------------
    ...
    t = 0; dt = 0.001; maxIt = 60;  %These are good combination
    g = 9.81;
    %---------------------- Initial Grid --------------------------------------
    ...
    for i = 1:6    %Try different level here. Maximum level is 6 to get a fast computation.
...
%      patch(x',y',[h;h;h],tcolor); %pause(0.001)%; view(3) %colorbar;
    ...
    NEP = (1.0/dt)*(Ent_after-U(4,:));
    subplot(1,2,1);
    showsolution(node,elem,h); view(3); zlim([0. 2.]);
    subplot(1,2,2);
    showsolution(node,elem,NEP); view(3); pause(0.001);       
    end
...
% 	saveas(gcf,'circular_dam_standard.fig')    
%     nt = size(elem,1)    
end
%-------------------- End of CIRCULAR DAM --------------------------------
